...
namespace PortalDMPlace.Controllers
{
    public class AdminController(DataContext context, AccountObject accountObject) : Controller
    {
        ...
        {
            // Apenas Administradores ou Desenvolvedores podem ver a lista de usuários
            if (!_accountObject.IsAtLeastAdmin) return Forbid();

            var usuarios = await _context.Accounts
                .AsNoTracking()
                .OrderByDescending(a => a.CreatedAt)
                .ToListAsync();

            return View("~/Views/Admin/Usuarios/Index.cshtml", usuarios);
        }

        [Authorize]
        [HttpGet("Admin/Usuarios/Criar")]
        public IActionResult CriarUsuario()
        {
            // Apenas nível Desenvolvedor pode acessar esta criação forçada
            if (!_accountObject.IsDev) return Forbid();
            
            return View("~/Views/Admin/Usuarios/Criar.cshtml");
        }

        [Authorize]
        [HttpPost("Admin/Usuarios/Criar")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CriarUsuario(Account model, string PlainPassword)
        {
            if (!_accountObject.IsDev) return Forbid();

            if (string.IsNullOrEmpty(PlainPassword))
            {
                ModelState.AddModelError("PlainPassword", "A senha é obrigatória.");
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (await _context.Accounts.AnyAsync(a => a.Username == model.Username))
                {
                    ModelState.AddModelError("Username", "Este nome de usuário já foi reivindicado.");
                    return View("~/Views/Admin/Usuarios/Criar.cshtml", model);
                }

                model.HashPassword = BCrypt.Net.BCrypt.HashPassword(PlainPassword);
                model.CreatedAt = DateTime.Now;

                _context.Accounts.Add(model);
                await _context.SaveChangesAsync();

                TempData["Sucesso"] = $"O usuário {model.Name} foi forjado no nível {model.AccountType}!";
